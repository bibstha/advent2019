...
class IntcodeComputer
  attr_accessor :memory, :current_instruction_pos

  class << self
    def load_from_disk(input)
      software = File.read(input).chomp.split(",").map(&:to_i)
      IntcodeComputer.new(software)
    end
  end
  ...
  def initialize(software)
    ...
    @tokenizer.input_iterator = @input.each
  end
  ...
  def input_iterator=(input_iterator)
